...
def run_resnet50_inference(image_path, output_dir="results", seg_model=None, cls_model=None):
    """
    Complete inference pipeline using ResNet50 as the classification backbone.
    Parity with VGG16 implementation.
    """
    # 1. Load Image
    try:
    ...
    if seg_model is None:
        seg_model_path = os.path.join(Config.RESNET50_DIR, "resnet50_seg.pth")
        seg_model = load_model(UNet, seg_model_path, Config.DEVICE)
    if cls_model is None:
        cls_model_path = os.path.join(Config.RESNET50_DIR, "resnet50_cls.pth")
    ...
    out_path = os.path.join(output_dir, "prediction_resnet50.jpg")
