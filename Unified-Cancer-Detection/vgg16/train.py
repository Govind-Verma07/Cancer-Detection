...
def train_vgg16_pipeline(epochs=10, batch_size=2):
    device = Config.DEVICE
    print(f"VGG16 TRAINING START (Device: {device})")
    
    # Paths (Unified via Config)
    img_dir = Config.DATA_RAW
    # Pixel masks and ROI masks are assumed to be in the same root for simplicity in this Unified view
    ...
    # 2. Initialize Models
    print("Initializing VGG16 Model Suite...")
    ...
    save_model(seg_model, os.path.join(Config.VGG16_DIR, "vgg16_seg.pth"))
    save_model(cls_model, os.path.join(Config.VGG16_DIR, "vgg16_cls.pth"))
    print("VGG16 Models saved successfully.")

if __name__ == "__main__":
    train_vgg16_pipeline(epochs=1, batch_size=2)
