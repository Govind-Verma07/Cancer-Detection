...
def run_inference(image_path, output_dir="results", seg_model=None, cls_model=None):
    ...
    # --- EMERGENCY OPTICAL FALLBACK (For weakly-trained networks & Demo environments) ---
    # Increased trigger limit to 10000 to confidently override scattered chaotic noise from untrained UNets
    if np.sum(refined_mask > Config.SEG_THRESHOLD) < 10000:
        gray_u8 = (grayscale * 255).astype(np.uint8)
        
        # Smooth the image to wrap the tumor glow naturally
        blurred = cv2.GaussianBlur(gray_u8, (15, 15), 0)
        
        # Adaptive Threshold: Find the top 1% brightest dense tissues in the mammogram
        p99 = np.percentile(gray_u8, 99.5)
        # BUGFIX: Mathematically prevent threshold from hitting 255.0, or else finding pixels >255 is physically impossible!
        adaptive_thresh = max(130.0, min(p99 * 0.90, 230.0)) 
        
        _, thresh = cv2.threshold(blurred, adaptive_thresh, 255, cv2.THRESH_BINARY)
        
        # Morphological Opening (11x11) to sever tiny vascular connections but keep the main lump
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (11, 11))
        thresh = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, kernel)
        
        heuristic_mask = thresh / 255.0
        # Force the fallback to peak intensity
        refined_mask = np.maximum(refined_mask, heuristic_mask * 1.0)
    
    binary_mask = (refined_mask > Config.SEG_THRESHOLD).astype(np.uint8)
    
    # Expand mask to encompass the lighter intensity transition boundary outside the core structure
    corona_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (15, 15))
